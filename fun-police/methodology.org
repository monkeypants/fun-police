...
- *Scope*: They often involve research, design, and updates to the framework files themselves (e.g., =fun-police/methodology.org=, =tech/systemPatterns.org=).
- *Structure*: While they reside in =spec/tasks.org=, their definition may be more goal-oriented than file-centric. The "Files to create/modify" section might be less prescriptive, focusing on the concepts to be documented rather than specific lines of code.
