...
** TODO Introduce ideas.org for creative and exploratory work
   ...
   **Files to create/modify**:
   - `spec/ideas.org`: Create the new file with an initial structure and state machine definition.
   - `fun-police/methodology.org`: Update to explain the role of `ideas.org` and how it feeds into `tasks.org`.
   - `fun-police/instructions.org`: Update to reflect the new file in the memory bank structure.
   - `spec/tasks.org`: Prune any visionary/non-actionable items that should be moved to `ideas.org`.
